...
## 🚀 วิธีเปิดใช้งานระบบพร้อมกันในคลิกเดียว (One-Click Launchers)

โครงงานนี้มาพร้อมไฟล์สคริปต์ `.bat` สำหรับเปิดทั้ง **โปรแกรมตรวจจับ** และ **หน้าจอแผนที่ GPS Geofence WebApp (100% Offline)** พร้อมกันในคลิกเดียว:

| ไฟล์เปิดระบบ | โหมดการทำงาน | สิ่งที่ระบบเปิดให้อัตโนมัติ |
|---|---|---|
| **`run_all_demo.bat`** | **🎮 โหมดจำลองนำเสนอ (Demo)** | 1. WebApp Server (Offline)<br>2. เปิด Browser `http://localhost:5000`<br>3. หน้าปัดเรือนไมล์ HUD + กล้อง + เสียงเตือน |
| **`run_all_real.bat`** | **⚡ โหมดฮาร์ดแวร์จริง (Real)** | 1. WebApp Server (Offline)<br>2. เปิด Browser `http://localhost:5000`<br>3. กล้องตรวจจับจริง (YOLO/Haar + GPS จริง + Relay) |
| **`run_all_real.sh`** | **🍓 สำหรับ Raspberry Pi (Linux)** | สคริปต์ Bash สำหรับรันบน Raspberry Pi OS พร้อมเปิดจออัตโนมัติ |
...
### วิธีรันบน PC / Laptop:
```bash
# เปิดทั้ง PC Demo + Geofence WebApp พร้อมกัน (แนะนำ)
run_all_demo.bat

# หรือรันเฉพาะ PC Demo เดี่ยวๆ
python pc_demo.py
```
